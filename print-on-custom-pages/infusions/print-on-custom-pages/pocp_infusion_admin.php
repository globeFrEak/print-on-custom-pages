...
//$pocp_ckbox = (isset($_POST['pocp_ckbox'])) ? 1 : 0; 
//$pocp_config = (isset($_POST['pocp_config'])) ? $_POST['pocp_config'] : "1";
//$pocp_title = $_GET['title'];
...
if (isset($_POST['save'])) {
	...
	pocp_title = '" . $pocp_title . "',
	pocp_config = '" . stripinput($pocp_config) . "',
	pocp_ckbox = '" . stripinput($pocp_ckbox) . "'");*/
	
	/*foreach ($_POST['data'] as $field_id => $field_value) {
    $q_update[] = "INSERT INTO ". DB_PRINT_ON_CUSTOM_PAGES ."(pocp_title, pocp_config, pocp_ckbox) VALUES ('".$field_value."','".$pocp_config."','".$pocp_ckbox."')";
    }
	
    foreach ($q_update as $qq) { $temp_result = dbquery($qq); }*/
...
if (dbrows($result) > 0) {
    ...
    while ($data = dbarray($result)) {
        ...
        echo "
		<tr>
		<td class='$cell_color' align='center'><input type='hidden' name='title[]'>" . $data['page_title'] . "</td>";
