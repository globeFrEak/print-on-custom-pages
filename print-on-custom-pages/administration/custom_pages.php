...
if (isset($_POST['save'])) {
	...
	//
	$print = isset($_POST['page_print']) ? "1" : "0";
	//
	if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
		if (isset($_POST['page_id']) && isnum($_POST['page_id'])) {
			...
			//
			$result = dbquery("SELECT * FROM " . DB_PRINT_ON_CUSTOM_PAGES . " WHERE pocp_title ='".$page_title."'");
			$data = dbarray($result);
			if (!empty($data['pocp_title'])){
			$result = dbquery(
				"UPDATE ".DB_PRINT_ON_CUSTOM_PAGES." SET
					pocp_title='".$page_title."',
					pocp_ckbox='".$print."'
				WHERE pocp_title='".$_POST['page_title']."'"
			);
			} else {
			$result = dbquery(
				"INSERT INTO ".DB_PRINT_ON_CUSTOM_PAGES." (
					pocp_title, pocp_config, pocp_ckbox
				) VALUES (
					'".$page_title."', '1', '".$print."'
				)"
			);
	...
	$result1 = dbquery("SELECT page_title FROM ".DB_CUSTOM_PAGES." WHERE page_id='".$_POST['page_id']."'");
	$data = dbarray($result1);
	$result = dbquery("DELETE FROM ".DB_PRINT_ON_CUSTOM_PAGES." WHERE pocp_title='".$data['page_title']."'");
	...
	if (isset($_POST['preview'])) {
		...
		$print = isset($_POST['page_print']) ? " checked='checked'" : "";
		if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
	...
	if (isset($_POST['edit']) && (isset($_POST['page_id']) && isnum($_POST['page_id']))) {
		...
		if (dbrows($result)) {
			$data = dbarray($result);
			$result1 = dbquery("SELECT pocp_ckbox FROM ".DB_PRINT_ON_CUSTOM_PAGES." WHERE pocp_title='".$data['page_title']."'");
			...
			$print = ($data1['pocp_ckbox'] == "1" ? " checked='checked'" : "");
			$addlink = "";
		} else {
	...
	// Druckoption auf custom_pages aktivieren
	include INFUSIONS."print-on-custom-pages/locale/German.php";
	echo "<br />\n";
	echo "<label><input type='checkbox' name='page_print' value='1'".$print." /> ".$locale['pocp004']."</label>\n";
	//
	echo "</td>\n</tr>\n";
